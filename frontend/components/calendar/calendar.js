...
function createCourseCard(course) {
  ...
  card.innerHTML = `
    ...
    <div class="text-xs mt-1">${course.statut}</div>
    <div class="text-xs">${course.classRoom}</div>
  `;
  card.onclick = () => showCourseDetailsModal(course);
  return card;
}
...
function createCalendarBody(config, courses) {
  ...
  grid.className = "grid min-w-[800px]";
  grid.style.gridTemplateColumns = `100px repeat(${config.days.length}, 1fr)`;

  // En-tête des jours
  const daysHeader = document.createElement("div");
  daysHeader.className = "col-span-full grid grid-cols-7 bg-base-200";
  ...
  config.days.forEach((day) => {
    ...
    dayCell.className =
      "p-2 text-center font-bold border-r border-b border-base-300";
  ...
  for (
    ...
    timeCell.className =
      "p-2 text-right border-r border-b border-base-300 bg-base-100";
    ...
    config.days.forEach((day, dayIndex) => {
      const dayCell = document.createElement("div");
      dayCell.className =
        "p-1 border-r border-b border-base-300 relative min-h-[80px]";
      ...
      courses
        ...
        .forEach((course) => {
          dayCell.appendChild(createCourseCard(course));
        });
...
export function createCourseCalendar(initialCourses = [], options = {}) {
  ...
  calendar.className = "bg-white rounded-lg shadow overflow-hidden h-[80vh]";
  ...
  // Body
  const body = createCalendarBody(config, initialCourses);
  calendar.appendChild(body);

  // Navigation si professorId est fourni
  if (config.professorId) {
    setupCalendarNavigation(calendar, config.professorId);
  }

  return calendar;

  function setupCalendarNavigation(calendarElement, profId) {
    ...
    const updateWeek = async (offset) => {
      ...
      try {
        const courses = await getProfessorWeeklyCourses(profId, offset);
        ...
        // Remplacer le corps
        const oldBody = calendarElement.querySelector("div:last-child");
        if (oldBody) {
        ...
        const body = calendarElement.querySelector("div:last-child");
        if (body) {
          body.innerHTML = `
