 ...
 */
export function createCoursCards(config) {
 ...
 */
export function updateCoursCardsData(
  containerId,
  newData,
  currentPage = 1,
  onAction = null
...
function createCoursCard(cours, actionsConfig, containerId) {
  ...
  card.dataset.id = cours.id_cours;
  ...
  moduleTitle.textContent = cours.module.libelle;
  cardHeader.appendChild(moduleTitle);
  // Corps de la card
  const cardBody = document.createElement("div");
  cardBody.className = "p-4 relative";

  // Informations sur le professeur avec avatar
  const profInfo = document.createElement("div");
  profInfo.className = "flex items-center gap-2";

  // Avatar du professeur
  const profAvatar = document.createElement("div");
  profAvatar.className = "avatar";

  const mask = document.createElement("div");
  mask.className = "mask mask-squircle h-7 w-7";

  const avatarImg = document.createElement("img");
  avatarImg.src = cours.professeur.utilisateur.avatar || "/default-avatar.png";
  avatarImg.alt = `${cours.professeur.utilisateur.prenom} ${cours.professeur.utilisateur.nom}`;

  const profName = document.createElement("span");
  profName.className = "text-sm";
  profName.textContent = `${cours.professeur.utilisateur.prenom} ${cours.professeur.utilisateur.nom}`;

  mask.appendChild(avatarImg);
  profAvatar.appendChild(mask);
  profInfo.appendChild(profAvatar);
  profInfo.appendChild(profName);

  // Informations sur les classes
  const classesInfo = document.createElement("div");
  classesInfo.className = "flex items-center py-2";

  const classesIcon = document.createElement("i");
  classesIcon.className = "ri-government-line mr-2 text-gray-500";

  const classesText = document.createElement("span");
  classesText.className = "text-sm";
  classesText.textContent = cours.classes.map((c) => c.libelle).join(", ");

  classesInfo.appendChild(classesIcon);
  classesInfo.appendChild(classesText);

  // Informations sur le semestre
  const semestreInfo = document.createElement("div");
  semestreInfo.className = "flex items-center mb-2";

  const semestreIcon = document.createElement("i");
  semestreIcon.className = "ri-calendar-line mr-2 text-gray-500";

  const semestreText = document.createElement("span");
  semestreText.className = "text-sm";
  semestreText.textContent = `${cours.semestre.libelle}`;

  semestreInfo.appendChild(semestreIcon);
  semestreInfo.appendChild(semestreText);
