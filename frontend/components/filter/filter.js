...
export function createCoursFiltersForRp(config) {
  ...
  semestreSelect.className = "select select-bordered w-full";
  semestreSelect.innerHTML = `
    <option value="">Toutes les semestres</option>
    ${semestres
      .map((s) => `<option value="${s.id}">${s.libelle}</option>`)
      .join("")}
  `;
  ...
  anneeSelect.innerHTML = `
    <option value="">Toutes les années</option>
    ${anneesScolaires
      .map((a) => `<option value="${a.libelle}">${a.libelle}</option>`)
      .join("")}
  `;
  anneeSelect.addEventListener("change", (e) => {
    onFilter({
  ...
  resetButton.addEventListener("click", () => {
    ...
    anneeSelect.value = "";
    onFilter({ search: "", semestre: "", annee: "" });
  });

  filtersGrid.appendChild(searchInput);
  filtersGrid.appendChild(semestreSelect);
  filtersGrid.appendChild(anneeSelect);
  filtersGrid.appendChild(resetButton);
  filtersContainer.appendChild(filtersGrid);
...
export function createCoursFiltersForEtudiant(config) {
  const { semestres = [], anneesScolaires = [], onFilter = () => {} } = config;
...
export function createJustificationFiltersForEtudiant(config) {
...
export function createAbsenceFiltersForEtudiant(config) {
  const { statut = [], onFilter = () => {} } = config;
  ...
  statutSelect.className = "select select-bordered w-full";
  statutSelect.innerHTML = `
    <option value="">Toutes les statut</option>
    ${statut
      .map((s) => `<option value="${s.libelle}">${s.libelle}</option>`)
      .join("")}
  `;
  statutSelect.addEventListener("change", (e) => {
    onFilter({
